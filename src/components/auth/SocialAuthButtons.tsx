...
export function SocialAuthButtons({ 
  ...
  const handleOAuthSignIn = async (strategy: OAuthStrategy) => {
    ...
    try {
      ...
      onError?.(error.message || `Failed to sign in with ${strategy}`)
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <button
        ...
        className={cn(
          ...
          "bg-white border-2 border-gray-200 rounded-xl py-3 px-4",
          "transition-all duration-200",
          "disabled:opacity-50 disabled:cursor-not-allowed",
          !loadingProvider && "hover:bg-gray-50 hover:border-gray-300 hover:shadow-md active:scale-[0.98]"
        )}
      ...
      <button
        ...
        className={cn(
          "w-full flex items-center justify-center gap-3",
          "bg-gray-900 text-white rounded-xl py-3 px-4",
          "transition-all duration-200",
          "disabled:opacity-50 disabled:cursor-not-allowed",
          !loadingProvider && "hover:bg-gray-800 hover:shadow-md active:scale-[0.98]"
        )}
        aria-label={`${mode === 'sign-in' ? 'Sign in' : 'Sign up'} with GitHub`}
      >
